...
<body>
  <div class="background-loader" id="loading-p">
    ...
    <h2 class="TAG"><B>MANAGE USER GROUP <?php echo $_GET['GroupName'];?></B></h2>
    <div class="table-container" id="loadpage">
        ...
        <a class="btn btn-warning" style="float:left;margin-right:20px;" onclick="Addusers()">
        <i class="fa fa-plus"></i> Add Users</a>
        <?php
        include './auth/subClass.php';
        $SCADA = new SCADA();
        if($SCADA->checkRoles("2") == 'error'){
            header("Location: ./Index.php?PermissionError=403");
        }else{
            echo $SCADA->getUser($_GET['GroupUsesr']);
        }
        ?>
    </div>
</body>
