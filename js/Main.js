...
$(function () {
  $("#cboTemplate1").on("change", function () {
    const data_val = $(this).val();
    $.ajax({
      url: "../src/auth/PostFile.php",
      type: "post",
      data: { tmplID: data_val },
      success: function (result) {
        if(result !== ''){
          document.getElementById("id_tmp").value = data_val;
...
const SetText = async (arr,time) => {
    window.location.href = "Index.php?SetArray=" + encodeURIComponent(JSON.stringify(arr)) + "&Time=" + time;
}
...
const SaveTemplate = async () => {
  ...
  document.getElementById('selected_tag').value = tag.value;
}
